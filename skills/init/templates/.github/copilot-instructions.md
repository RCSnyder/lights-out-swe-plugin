...
**This repo uses the `lo-swe` plugin.** The pipeline's agents, skills, and prompts are provided by the plugin and are not stored in this repo. Only the project-root harness lives here: `.github/copilot-instructions.md`, `preferences.md`, `scaffolding/`, and `docs/input/`. `preferences.md` is customized per project.
...
The `lo-swe` plugin provides the phase prompts (`/lo-swe:expand`, `/lo-swe:design`, `/lo-swe:analyze`, `/lo-swe:build`, `/lo-swe:review`, `/lo-swe:reconcile`, `/lo-swe:verify`, `/lo-swe:deploy`, `/lo-swe:iterate`), specialist agents (Analyze, Review, Reconcile, Verify, Explore), and execution skills (including `build-discipline`).

## First Commit

Before the first `git add -A`, ensure `.gitignore` covers your stack. The default written by `/lo-swe:init` is:
...
3. **(Optional) Stack audit**: If the input docs describe an unfamiliar domain, many external integrations, or if the user requests it, run `/lo-swe:audit-stack` to validate that preferences.md stack choices are orthodox and right-sized for the problem. Skip for projects that clearly fit the default stack.
...
- During BUILD, ITERATE -> BUILD, and verify-fix cycles, follow the `build-discipline` skill provided by the `lo-swe` plugin (invokable as `/lo-swe:build-discipline`). It defines slice sizing, anti-rationalization checks, debugging, and change summaries.
- Read `scaffolding/readiness.md` before writing code. If it is missing or says `NOT READY`, stop and run `/lo-swe:analyze`.
...
- For house/skyscraper projects: define project-specific agents in this repo's `.github/agents/` as clear roles emerge from the architecture. These are project code, not scaffolding, and are separate from the lo-swe plugin's agents.

Then run the **post-build gate** before proceeding.

### Phase 3.5: REVIEW

After BUILD passes its gate, run `/lo-swe:review` (which dispatches to the Review agent) to audit the code before reconciliation and verification.
...
After REVIEW passes its gate — and before VERIFY — run `/lo-swe:reconcile` (which dispatches to the Reconcile agent) to detect and fix drift between scaffolding documents and the actual codebase.
...
- Run all tests via `/lo-swe:verify` (which dispatches to the Verify agent — read-only, cannot edit code)
...
1. User adds feedback/requirements to `docs/input/` and runs `/lo-swe:iterate`
...
- Follow the `build-discipline` skill provided by the `lo-swe` plugin (invokable as `/lo-swe:build-discipline`) as the default execution workflow.
...
This system runs as an **autonomous loop**, not a step-by-step wizard. The human writes the program (these instructions + `preferences.md`), the agent runs the loop, the gates are the metric.
...
When you come back in the morning, `scaffolding/log.md` tells you everything that happened.
...
When the user says `/lo-swe:iterate` on a shipped project:
