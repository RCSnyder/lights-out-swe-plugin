...
> **Custom agent**: This workflow is normally driven by the `Lights Out SWE: Verify` custom agent. Switch to that agent (or run from the main agent if the dropdown is unavailable) before executing the steps below.
...
2. Control returns to the **main agent** (with edit capability) to fix the issues
   - Before fixing anything, load `skills/build-discipline/SKILL.md`
