...
The plugin ships only the reusable pieces: agents, skills, and commands. The project-root harness (`copilot-instructions.md`, `preferences.md`, `scaffolding/`, `docs/input/`) is written into a fresh repo by [`/lo-swe:init`](commands/init.md).
...
| Agent                            | Purpose                                                                                                              |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| [analyze](agents/analyze.md)     | Pre-BUILD admission control. Maps `AC-*` to proofs, separates truths from assumptions, blocks silent simplification. |
| [explore](agents/explore.md)     | Read-only codebase Q&A. Recovers context, locates implementations, answers without edit risk.                        |
| [reconcile](agents/reconcile.md) | Cross-checks scaffolding documents against the actual codebase. Detects and fixes drift.                             |
| [review](agents/review.md)       | Reviews built code for correctness, readability, architecture, security, and performance.                            |
| [verify](agents/verify.md)       | Runs tests, validates AC coverage, checks security and deployment readiness.                                         |
...
- File names inside `agents/`, `commands/`, and `skills/` are bare action names (`reconcile.md`, not `lo-swe-reconcile.md`). The plugin name is prepended by the loader; repeating it produces ugly invocations like `/lo-swe:lo-swe-reconcile`.
- Identifiers are plain kebab-case. Names containing `/` or `:` silently fail to load.
- Display `name:` in frontmatter of agents and commands: `Lights Out SWE: <Title>`, so the agent picker shows ownership at a glance. Skills use a bare identifier in `name:` (e.g. `build-discipline`).
